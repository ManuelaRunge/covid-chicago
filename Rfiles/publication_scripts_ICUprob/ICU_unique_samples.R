...
dat_withSamples <- dat %>%
                  filter(region=="Region 1" &  
                           capacity_multiplier %in% c(0.4,0.6, 0.8) & 
...
pdf(file.path(sim_dir, "ICU_samples_plots","samples_plots_v3.pdf"))
for(sample_var in sample_vars){
  ...
  dat_withSamples_sub <- dat_withSamples %>% 
    filter(sample_num  %in% c(3))
  
  pp <- ggplot(data=subset(dat_withSamples, reopen="100perc"))+
    geom_boxplot(aes(x=capacity_multiplier_fct , y=plotVar, group=capacity_multiplier), width=0.5)+
    geom_jitter(aes(x=capacity_multiplier_fct , y=plotVar, col=sample_num),width=0.1, alpha=0.3)+
    geom_point(data=dat_withSamples_sub,
               aes(x=capacity_multiplier_fct , y=plotVar),fill="orange",shape=21,size=2.3, alpha=0.9)+
    theme(legend.position = "none")+
   # geom_hline(aes(yintercept = unique(dat_withSamples_sub$plotVar) ),col="orange") + 
    labs(title=paste0(sample_var,"\n"), caption="orange point = sample_num 3",y=sample_var)+
    facet_wrap(~ reopen)+
    customTheme
  print(pp)
  rm(pp)
