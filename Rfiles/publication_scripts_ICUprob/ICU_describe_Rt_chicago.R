...
subregions <- c( "covidregion_11")
dat$region <- factor(dat$geography_modeled, levels = paste0("EMS-", c(11)), labels = paste0("Region ", c(11)))

dat <- dat %>%
  filter(geography_modeled %in% paste0("EMS-", c(11))) %>%
  f_get_scenVars()
...
f_save_plot(
  plot_name = paste0("Rt_timeline_counterfactual_reg11"), pplot = pplot,
  plot_dir = file.path(sim_dir, "ICU_Rt_plots"), width = 12, height = 8
)
...
dat_scenarios$date <- dat_scenarios$date+61

#  capacity_multiplier_fct %in% c('0', '20', '40', '60', '80','100') & 
#c("#fa9fb5","#f768a1","#dd3497","#e7298a","#7a0177","#49006a")
pplot <- ggplot(data = subset(dat_scenarios,
  capacity_multiplier_fct %in% c('20') & 
  reopen=="100perc" & 
  ...
  geom_ribbon(aes(
    ...
    fill = rollback,  alpha = rollback, group = interaction(rollback, reopen_fct2) )) +
  geom_line(aes(
    x = date, y = rt_median, col = rollback),size=1.2) +
   scale_color_manual(values = rev(c("#ee2a7b","#f15495","#f47faf","#f8a9ca"))) + 
  scale_fill_manual(values =rev( c("#ee2a7b","#f15495","#f47faf","#f8a9ca"))) +
  scale_alpha_manual(values = c(0.4,0.40,0.45,0.50,0.55)) +
    geom_hline(yintercept = 1) +
...
f_save_plot(
  plot_name = paste0("Rt_mitigation"),
  pplot = pplot,
  plot_dir = file.path(sim_dir, "ICU_chicago_plots"), width = 12, height = 5
)
