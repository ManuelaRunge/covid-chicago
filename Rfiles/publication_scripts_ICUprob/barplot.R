...
pplot <- ggplot(data = plotdat) +
  ...
  geom_bar(data=subset(dat,(delay == "counterfactual" |delay == delay_val & rollback == rollback_values[4])),
           aes(x = capacity_multiplier_fct2, y = critical_median, group = reopen, fill = reopen),
           stat = "identity", position = position_dodge(width = 0.91), width = 0.7
  ) +
  
  geom_errorbar(aes(x = capacity_multiplier_fct2, ymin =critical_95CI_lower, ymax = critical_95CI_upper, group=reopen),
    position = position_dodge(width = 0.91), width = 0
  ) +
  labs(y="Predictd peak in ICU census\nuntil end of December", 
       x="Trigger threshold\n (% of ICU capacity)")+
  geom_hline(aes(yintercept =avg_resource_available), linetype="dashed", col=capacity_col,size=1) +
  geom_text(aes(x=1.5, y=avg_resource_available+(avg_resource_available*0.5), label="ICU capacity"), col=capacity_col)+
