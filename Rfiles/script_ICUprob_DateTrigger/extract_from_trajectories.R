...
for (exp_name in exp_names) {
  ...
  for (reg_nr in c( 11)) {
    ...
    samplesDat <-  fread(file.path(sim_dir, exp_name , "sampled_parameters.csv")) %>% select(scen_num, time_of_trigger)
    dat <- f_addVar(dat, samplesDat)
    
    dat <- dat %>%
      select(region ,date, exp_name, time_of_trigger, sample_num, scen_num, crit_det) %>%
      left_join(capacityDat, by = "region") %>%
      filter(date >= as.Date("2020-09-01")) %>%
      group_by(exp_name, time_of_trigger, sample_num, scen_num) %>%
    ...
    peakDate <- dat %>%
     ...
     # filter(peak_date <= as.Date("2020-12-31")) %>%
      ...
      select(exp_name, time_of_trigger, sample_num, scen_num, peak_date) %>%
      unique()
    ...
    exceedDate <- dat %>%
     ...
     # filter(exceed_date_to <= as.Date("2020-12-31")) %>%
      ...
      group_by(exp_name, time_of_trigger, sample_num, scen_num) %>%
      select(exp_name, time_of_trigger, sample_num, scen_num, exceed_date_from, exceed_date_to, exceed_diff) %>%
      unique()


    plotdat <- dat %>%
      # filter(time_of_trigger %in% c(0.4, 0.6, 0.8, 1)) %>%
      f_addVar(peakDate) %>%
      f_addVar(exceedDate) %>%
      group_by(exp_name, time_of_trigger, sample_num, scen_num) %>%
      mutate(
    ...
    #   select(exp_name, time_of_trigger, sample_num, scen_num, avg_resource_available, crit_det, date) %>%
    #   mutate(trigger_capacity_val = avg_resource_available * time_of_trigger) %>%
    #   filter(crit_det > trigger_capacity_val) %>%
    #   group_by(exp_name, time_of_trigger, sample_num, scen_num, avg_resource_available, trigger_capacity_val) %>%
...
#     rm(trigger_capacity_Dat)
    ...
    plotdat_peak <- plotdat %>% ungroup() %>% group_by(exp_name, region, time_of_trigger, scen_num) %>% filter(crit_det==max(crit_det))
