...
f_generateTimeline_plot <- function(selected_region, addLastWeek = T, plot_stop_date = today,
                                    ...
                                    changepoint2 = as.Date("2020-10-07")) {
  ...
  p1 <- ggplot(data = dat) +
    ...
    labs(
      title = plot_title,
      subtitle = paste0("Estimated Rt using NU's COVID-19 transmission model \nEstimated Rt for ",simdate_label,": ", round(currentRt$rt_median, 3), " (95%CI: ", round(currentRt$rt_lower, 3), " - ", round(currentRt$rt_upper, 3), ")"),
  ...
  if (addLastWeek) {
    ...
    p1 <- p1 + geom_line(data = dat_lastweek, aes(x = date, y = rt_median), col = "deepskyblue4", size = 1, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = today - 15, y = ylastweek - 0.03, label = "last week's fit", col = "deepskyblue4", size = 4, alpha = 0.5) +
      annotate("text", x = today - 15, y = ythisweek + 0.09, label = "this week's fit", col = "deepskyblue4", size = 4, alpha = 0.9)
  }
...
for (selected_region in selected_regions) {
  f_generateTimeline_plot(selected_region = selected_region, addLastWeek=TRUE)
}
