...
reopeningdate = as.Date("2020-09-09")

## ================================================
###  Figure 3 - HS scenarios - no contact tracing
## ================================================

combineScenarioDats=FALSE
if (combineScenarioDats) {
  ...
  f_getPredDat <- function(expDIR) {
    ...
    paramvars <- paramvars[paramvars !="Ki_t_All"]
  ...
  exp_names <- c(
   paste0(simdate, "_IL_reopen_changeTD"), 
   paste0(simdate,"_IL_reopen_counterfactual"), 
   paste0(simdate,"_IL_reopen_HS40changeTD"), 
   paste0(simdate,"_IL_reopen_HS40"), 
   paste0(simdate,"_IL_reopen_HS80changeTD"), 
  ...
  predDatHS <- predDat %>% bind_rows()
  RtDatHS <- RtDat %>% bind_rows()
  
  table(predDatHS$scenario)
  table(predDatHS$param)
  table(RtDatHS$scenario)
  
  table(predDatHS$region)
  predDatHS$region[is.na(predDatHS$region)] <- "Illinois"
  predDatHS$region <- tolower( predDatHS$region)
 ...
 load(file.path(simulation_output, "contact_tracing",simdate,"predDatHS.Rdata"))
  #load(file.path(simulation_output, "contact_tracing/",simdate,"RtDatHS.Rdata"))
}

capacityDat <- load_new_capacity(selected_ems = tolower(unique(predDatHS$region))) %>% rename(region=geography_name)
...
#### ---------------------------
capacity_long <- capacityDat %>% pivot_longer(cols = -c("geography_name"), names_to = "param", values_to = "capacity") %>%
...
predDatHS$scenario_fct <- factor(predDatHS$scenario,
                                 levels = c("counterfactual", "HS40", "changeTD","HS80", "HS40changeTD", "HS80changeTD"),
                                 labels = c(
...
predDatHS <- predDatHS %>% left_join(capacityDat, by="region") 
...
predplot <- predDatHS %>%
  ...
  ggplot() +
  theme_cowplot() +
  geom_ribbon(aes(x=date , ymin=q2.5 , ymax=  q97.5 , fill=scenario_fct , group=scenario_fct),size=1, alpha=0.3) +
  geom_ribbon(aes(x=date , ymin=q25 , ymax=  q75 , fill=scenario_fct , group=scenario_fct),size=1, alpha=0.3) +
  geom_line(aes(x = date, y = median.val, color = scenario_fct, group = scenario_fct), size = 1.3) +
  geom_hline(aes(yintercept = capacity$icu_available[capacity$geography_name=="illinois"]), linetype = "dashed", color = "black", size = 0.7) +
...
predplot_reg <- predDatHS %>%
  ...
  ggplot() +
  theme_cowplot() +
  geom_ribbon(aes(x=date , ymin=q2.5 , ymax=  q97.5 , fill=reopening_multiplier_4 , group=reopening_multiplier_4),size=1, alpha=0.3) +
  geom_ribbon(aes(x=date , ymin=q25 , ymax=  q75 , fill=reopening_multiplier_4 , group=reopening_multiplier_4),size=1, alpha=0.3) +
  geom_line(aes(x = date, y = median.val, color = reopening_multiplier_4, group = reopening_multiplier_4), size = 1.3) +
  geom_hline(aes(yintercept = icu_available), linetype = "dashed", color = "black", size = 0.7) +
  geom_hline(yintercept = c(-Inf, Inf)) +
  geom_vline(xintercept = c(-Inf, Inf)) +
...
if(SAVE){
  ggsave(paste0("HS_scenarios_timeline_2.pdf"),
         plot = predplot, path = file.path(pdfdir), width = 12, height =5,  device = "pdf"
  )
}
