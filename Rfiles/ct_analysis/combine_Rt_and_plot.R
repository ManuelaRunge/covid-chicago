...
f_combineRtdats <- function() {
  ...
  if (file.exists(file.path(Rt_dir, "1_estimated_Rt.Rdata"))) load(file.path(Rt_dir, "1_estimated_Rt.Rdata"))
  if (file.exists(file.path(Rt_dir, "1_temp_Rt_tempdat_All.Rdata"))) load(file.path(Rt_dir, "1_temp_Rt_tempdat_All.Rdata"))
  Rt_dat <- Rt_tempdat_All
  rm(Rt_tempdat_All)

  for (i in c(2:11)) {
    if (file.exists(file.path(Rt_dir, paste0(i, "_estimated_Rt.Rdata")))) load(file.path(Rt_dir, paste0(i, "_estimated_Rt.Rdata")))
    if (file.exists(file.path(Rt_dir, paste0(i, "_temp_Rt_tempdat_All.Rdata")))) load(file.path(Rt_dir, paste0(i, "_temp_Rt_tempdat_All.Rdata")))
  ...
  Rt_dat2 <- Rt_dat2 %>% mutate(
    meanRtLE1 = ifelse(Median < 1, 1, 0),
    Date = as.Date(Date)
  )
