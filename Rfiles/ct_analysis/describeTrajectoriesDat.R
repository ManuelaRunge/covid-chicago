...
f_runDescriptivePlots <- function(){
  #selected_ems <-c(1:11)
  ...
  capacitysum <- load_capacity("allsum")
  ...
  trajectoriesDatAggr = f_aggrDat(subdat, c("Date","region", "region_label", "outcome", "ICU_availability", "ICU_capacity"), "value")
  
  
  subdatAggr = subdat %>% 
    dplyr::group_by(region,  scen_num, outcome ) %>% 
    dplyr::mutate(valueMax = max(value, na.rm=TRUE)) %>%
    filter(valueMax<=ICU_availability) %>% 
    f_aggrDat( c("Date","region", "region_label", "outcome", "ICU_availability", "ICU_capacity"), "value")
  ...
  ggplot(data = subset(subdat, outcome=="critical")) +
    ...
    geom_hline(aes(yintercept=ICU_capacity)) + 
    geom_hline(aes(yintercept=ICU_availability)) + 
  ...
  pplot <- ggplot(data = subset(trajectoriesDat, Date == max(Date)), aes(x=grpvar, y =crit_cumul_All, group=grpvar )) +
    ...
    #geom_hline(yintercept=capacitysum$ICU_capacity) + 
    #geom_hline(yintercept=capacitysum$ICU_availability) + 
  ...
  for (ems in c(1:11)) {
    ...
    capacitySub <- capacity[capacity$ems==ems,] 
    ...
    l_plot <- ggplot(data = subset(trajectoriesDat)) +
      ...
      geom_hline(yintercept=capacitySub$ICU_capacity) + 
      geom_hline(yintercept=capacitySub$ICU_availability) + 
      scale_color_viridis(discrete = TRUE) +
      labs(title=paste0("covid region  ", ems), subtitle="" , color = "detection_success") +
      customThemeNoFacet +
      scale_x_date(breaks = "1 month", labels = date_format("%b")) 
    ...
    l_plot2 <- ggplot(data = subset(trajectoriesDatAggr)) +
      ...
      geom_line(aes(x = Date, y = mean.val), col="deepskyblue4",size=1.3) +
      scale_color_viridis(discrete = TRUE) +
      labs(title=paste0("covid region  ", ems), subtitle="" ,y="critical") +
      customThemeNoFacet +
      scale_x_date(breaks = "1 month", labels = date_format("%b")) 
