...
groupvars <- c("time", "Date", "startdate", "scen_num", "sample_num", "run_num", "time_to_detection", "detection_success", "isolation_success")
...
if (tryMinValue) {
  ...
  testplot1 <- ggplot(data = df) +
    ...
    scale_fill_viridis(option = "C", discrete = TRUE, direction = -1) +
    geom_smooth(data = threshold_param2, aes(
      x = detection_success_fct, y = isolation_success, col = as.factor(time_to_detection),
      fill = as.factor(time_to_detection)
    ), se = FALSE, method = "lm", formula = y ~ poly(x, 2, raw = TRUE)) +
    scale_color_viridis(option = "C", discrete = TRUE, direction = -1) +
    customThemeNoFacet +
    scale_x_continuous(lim = c(0, 1), breaks = seq(0, 1, 0.1), labels = seq(0, 1, 0.1) * 100, expand = c(0, 0)) +
    scale_y_continuous(lim = c(0, 1), breaks = seq(0, 1, 0.1), labels = seq(0, 1, 0.1) * 100, expand = c(0, 0)) +
