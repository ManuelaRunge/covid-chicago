...
outdir <- file.path("C:/Users/mrm9534/Box/MR_archive/testfigures")
...
load_sim_data <- function(exp_names) {
  ...
  for (exp_name in exp_names) {
    ...
    outcomeVars <- paste0("critical_det_", outcomeVars)
    ...
    simdat <- trajectoriesDat %>%
      ...
      mutate(
        ...
        exp_label = gsub("20200831_IL_regreopen", "", exp_name)
      ) %>%
...
if (trajectoriesPlot) {
  ...
  trajectoriesPlot_2 <- function() {
    p1 <- ggplot(data = subset(simdat, geography_name == "illinois" & capacity_multiplier == 0.8 & delay == "none")) +
    ...
    p2 <- ggplot(data = subset(simdat, geography_name == "illinois" & capacity_multiplier == 0.8 & rollback == "sm4")) +
    ...
    p3 <- ggplot(data = subset(simdat, geography_name == "illinois" & !is.na(capacity_multiplier2) & delay == "none" & rollback == "sm4")) +
