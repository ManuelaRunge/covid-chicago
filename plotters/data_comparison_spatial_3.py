...
def plot_sim_and_ref(df, ems_nr, ref_df, channels, data_channel_names, titles, first_day=date(2020, 2, 22),
    ...
    for c, channel in enumerate(channels):
        ...
        mdf = df.groupby(['time','recovery_time_crit'])[channel].agg([np.mean, CI_2pt5, CI_97pt5, CI_25, CI_75]).reset_index()

        for i, rtc in enumerate(mdf['recovery_time_crit'].unique()):
            mdf_sub = mdf[mdf['recovery_time_crit'] == rtc]
