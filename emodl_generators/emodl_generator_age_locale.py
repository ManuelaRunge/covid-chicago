...
def write_N_population(ageList, regionList, pop_dic):
    ...
    for age_i in ageList:
        temp_str = "\n(param N_" + age_i + " (+ " + repeat_string_by_grp(fixedstring='N_' + age_i + '_',
                                                                         grpList1=regionList, grpList2=None) + "))"
...
def write_All(ageList, regionList, observeLevel='primary'):
    obs_primary_All_str = ""
    obs_primary_All_str = obs_primary_All_str + "\n(observe susceptible_All (+ " + repeat_string_by_grp('S_', ageList,
                                                                                                        regionList) + "))"
    ...
    obs_primary_All_str = obs_primary_All_str + "\n(observe deaths_det_All (+ " + repeat_string_by_grp('D3_det3_',
                                                                                                       ageList,
                                                                                                       regionList) + "))"
    obs_primary_All_str = obs_primary_All_str + "\n(observe deaths_All (+ " + repeat_string_by_grp('deaths_', ageList,
                                                                                                   regionList) + "))"
    ...
    obs_secondary_All_str = obs_secondary_All_str + "\n(observe exposed_All (+ " + repeat_string_by_grp('E_', ageList,
                                                                                                        regionList) + "))"
    ...
    obs_tertiary_All_str = obs_tertiary_All_str + "\n(observe seroprevalence_All (+ " + repeat_string_by_grp(
        'seroprevalence_', ageList, regionList) + "))"
    obs_tertiary_All_str = obs_tertiary_All_str + "\n(observe prevalence_det_All (+ " + repeat_string_by_grp(
        'prevalence_det_', ageList, regionList) + "))"
    obs_tertiary_All_str = obs_tertiary_All_str + "\n(observe seroprevalence_det_All (+ " + repeat_string_by_grp(
        'seroprevalence_det_', ageList, regionList) + "))"
...
def write_interventions(regionList, ageList, total_string, scenarioName, expandModel, change_testDelay=None,
...
(observe triggertime_{regionout} time_of_trigger_{region})
    ...
    for region in regionList:
        regionout = sub(region)
        temp_str = """
(param d_Sym_{region} @d_Sym_{region}@)
(observe d_Sym_t_{regionout} d_Sym_{region})

(time-event d_Sym_change1 @d_Sym_change_time_1@ ((d_Sym_{region} @d_Sym_change1_{region}@)))
(time-event d_Sym_change2 @d_Sym_change_time_2@ ((d_Sym_{region} @d_Sym_change2_{region}@)))
(time-event d_Sym_change3 @d_Sym_change_time_3@ ((d_Sym_{region} @d_Sym_change3_{region}@)))
(time-event d_Sym_change4 @d_Sym_change_time_4@ ((d_Sym_{region} @d_Sym_change4_{region}@)))
(time-event d_Sym_change5 @d_Sym_change_time_5@ ((d_Sym_{region} @d_Sym_change5_{region}@)))
            """.format(regionout=regionout, region=region)
...
(time-event gradual_reopening4 @gradual_reopening_time4@ ((Ki_{region} Ki_back4_{region})))
    ...
    improveHS_str = define_change_detection_and_isolation(grpList=regionList,
    ...
    contactTracing_str = define_change_detection_and_isolation(grpList=regionList,
                                                               reduced_inf_of_det_cases=True,
                                                               d_As=True,
                                                               d_P=True,
                                                               d_Sym_ct=False,
    ...
    contactTracing_improveHS_str = define_change_detection_and_isolation(grpList=regionList,
                                                                         reduced_inf_of_det_cases=True,
                                                                         d_As=True,
                                                                         d_P=True,
                                                                         d_Sym_ct=True,
...
def generate_emodl(age_list, region_list, pop_dic, import_dic, import_dic2, file_output, expandModel, add_interventions,
                   observeLevel='secondary', add_migration=True, change_testDelay=None, trigger_channel=None):
    ...
    param_string = write_params(expandModel) + age_specific_param_string + param_string + write_observed_param(age_list,
                                                                                                               region_list) + write_N_population(
        age_list, region_list, pop_dic) + write_contact_matrix(age_list)
    if (add_migration):
        param_string = param_string + write_migration_param(region_list)

    functions_string = functions_string + write_All(age_list, region_list, observeLevel=observeLevel)
    ...
    if add_interventions != None:
        total_string = write_interventions(region_list, age_list, total_string, add_interventions, expandModel,
                                           change_testDelay, trigger_channel)
...
if __name__ == '__main__':
    ...
    age_region_pop = {
        ...
        'EMS_6': [{"age0to9": 78524, "age10to19": 92005, "age20to29": 119387, "age30to39": 96035, "age40to49": 94670,
